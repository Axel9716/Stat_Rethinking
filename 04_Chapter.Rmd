...
sigma ~ Uniform(0, 10) # Change 10 to 8 for 4M6
...
sim_height <- function(W, sigma) {
  ...
  b <- rnorm(length(W), 3.5, 1) # this was based off the mean and sd of H/W in Howell's data. Not a good idea
  H <- a + b*W
  return(H)
}
...
w_post <- quap(alist(
  ...
  a ~ dnorm(0,10),
  b ~ dnorm(3.5, 1),
  sigma ~ dunif(0, 50) #changed from dunif(0,10) so quap would run. In reality, my gen model is probably really bad
), data = list(H=H, W=W))
...
# linear regression using adult model in quap
h42_list <- alist(height ~ dnorm(mu, sigma),
                  mu <- a + b*weight,
                  a ~ dnorm(0, 10),
                  b ~ dnorm(3.5, 1),
                  sigma ~ dunif(0,10))

h42 <- quap(h42_list, data = d2)


# When weight is 0, average height is 57 cm
# Every 10 kg increase in weight, results in +27.7 cm of height on average
# 95% of height falls between +/- 16cm of the mean height
precis(h42)

# Almost same as above, but with histogram
post <- extract.samples(h42)
precis(post)



# Create regression line with interval
weight_seq <- seq(from = min(d2$weight), to = max(d2$weight), by = 1)
mu <- link(h42, data = data.frame(weight = weight_seq))

mu_mean <- apply(mu, 2, mean)
mu_PI <- apply(mu, 2, PI, prob = .89)


# Plot real data with regression line overlayed
plot(d2$weight, d2$height, col = col.alpha(rangi2, .5), xlab = "weight (kg)", ylab = "height (cm)")
mtext("Age < 18")
...
# The model does a bad job of predicting most weights because it doesnt follow the curvature 
# of the relationship between height and weight. It would be better to fit a model with 
# quadratic terms or use splines.
